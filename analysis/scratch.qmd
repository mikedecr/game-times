...
```{python}
regime_date = df.filter(col("play_time").gt(col("avg_time"))).get_column("date").min()
df = df.with_columns(
    censorship_regime = col("date").lt(regime_date)
)
```
...
def ts_plot(data: pl.DataFrame, *args) -> PlotSpec:
    ...
    return qplot(
        ...
        ggtitle(name),
        geom_vline(xintercept = regime_date, color = "gray"),
        *args
    )
```
...
scatterplot(
    tango,
    coord_fixed(xlim=(0, 400), ylim=(0, 400)),
    # scale_y_log10(),
    # scale_x_log10(),
    geom_line(aes(x = "avg_time", y = "avg_time"), color = "gray"),
    geom_smooth()
)
```
...
```{python}
max_avg = queens.get_column("avg_time").max())
scatterplot(
    queens,
    coord_fixed(xlim=(0, max_avg), ylim=(0, max_avg)),
)
```
...
def ratio_plot(data: pl.DataFrame, *args) -> PlotSpec:
    ...
    return qplot(
        ...
        geom_point(aes(y = "time_ratio")),
        ggtitle(name),
        geom_vline(xintercept = regime_date, color = "maroon"),
        *args
    )
```
...
```{python}
ratio_vs_avg_plot(tango, scale_x_log10(), scale_y_log10())
```

```{python}
ratio_vs_avg_plot(queens, scale_y_log10())
```
...
qplot(
    ...
    .with_columns(
        dayname = col("weekday").replace_strict(range(1, 8), ISO_weekday_order)
    ),
    aes(x = "avg_time", group = "weekday", color = "weekday"),
